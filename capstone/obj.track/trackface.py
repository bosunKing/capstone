...
while cap.isOpened():
    ...
    for (x, y, w, h) in faces:
        ...
        #roi_gray = gray[y : y + h, x : x + w]    #요약하면 코드는 그레이스케일 이미지( )에서 직사각형 관심 영역을 가져오고 
        #roi_color = frame[y : y + h, x : x + w]  #gray컬러 이미지( frame)에서 해당 영역을 가져옵니다. 이 접근 방식은 전체 이미지를 한 번에 처리하는 대신 특정 이미지 처리 또는 이미지의 지역화된 영역에 대한 분석을 수행하려는 경우에 자주 사용됩니다. 
